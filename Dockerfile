...
RUN buildDeps='git build-base libressl-dev libwebsockets-dev util-linux-dev hiredis-dev curl-dev libxslt docbook-xsl'; \
    chmod +x /run.sh && \
    mkdir -p /var/lib/mosquitto && \
    touch /var/lib/mosquitto/.keep && \
    mkdir -p /etc/mosquitto.d && \
    apk update && \
    apk add $buildDeps postgresql-dev hiredis libwebsockets libuuid c-ares-dev c-ares libressl curl ca-certificates && \
    ...
    make WITH_MEMORY_TRACKING=no WITH_SRV=no WITH_WEBSOCKETS=yes WITH_TLS_PSK=no && \
    make install && \
